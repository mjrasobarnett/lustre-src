 ...
 * first.
 *
 * \retval excess unused-lock count.
 */
static int ldlm_lock_add_to_lru(struct ldlm_lock *lock)
{
	struct ldlm_namespace *ns = ldlm_lock_to_ns(lock);
	int excess_lock_count;
	ENTRY;

	spin_lock(&ns->ns_lock);
	ldlm_lock_add_to_lru_nolock(lock);
	excess_lock_count = ns->ns_nr_unused - ns->ns_max_unused;
	spin_unlock(&ns->ns_lock);

	RETURN(excess_lock_count);
}
...
void ldlm_lock_decref_internal(struct ldlm_lock *lock, enum ldlm_mode mode)
{
	...
	if (!lock->l_readers && !lock->l_writers && ldlm_is_cbpending(lock)) {
		 ...
		 */
		excess_lock_count = ldlm_lock_add_to_lru(lock);
		unlock_res_and_lock(lock);

		LDLM_DEBUG(lock, "add lock into lru list: %d",
			   excess_lock_count);
		...
		/*
		 * If lru resize disabled, call ldlm_cancel_lru() when the number
		 * of unused lock exceeds maximum allowed number. Usually this
		 * only happens for extent lock with dirty pages, e.g. creating
		 * many files with a small amount of data quickly.
		 */
		if (!ns_connect_lru_resize(ns) &&
		    excess_lock_count > LDLM_LRU_EXCESS_LIMIT)
			ldlm_cancel_lru(ns, excess_lock_count, LCF_ASYNC,
					LDLM_LRU_FLAG_AGED);
	} else {
